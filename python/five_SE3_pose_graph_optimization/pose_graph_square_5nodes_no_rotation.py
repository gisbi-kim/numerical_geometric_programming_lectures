...
def main():
    ...
    graph.generate_node_at_graph("C", initial_pose=np.eye(4))
    graph.generate_node_at_graph("D", initial_pose=np.eye(4))
    graph.generate_node_at_graph("E", initial_pose=np.eye(4))
    ...
    # 나머지 노드에 대한 Prior는 없거나 필요에 따라 추가

    # 3. Between Factors 추가 (네모 형태로 연결)
    # A -> B: x로 1만큼 이동
    between_AB = create_pose_matrix(0, 0, 0, np.array([1, 0, 0]))
    graph.add_between_factor("A", "B", between_AB)

    # B -> C: y로 1만큼 이동
    between_BC = create_pose_matrix(0, 0, 0, np.array([0, 1, 0]))
    graph.add_between_factor("B", "C", between_BC)

    # C -> D: x으로 -1만큼 이동 (역방향)
    between_CD = create_pose_matrix(0, 0, 0, np.array([-1, 0, 0]))
    graph.add_between_factor("C", "D", between_CD)

    # D -> E: y으로 -1만큼 이동 (역방향)
    between_DE = create_pose_matrix(0, 0, 0, np.array([0, -1, 0]))
    graph.add_between_factor("D", "E", between_DE)

    # E -> A: 루프 클로저 (identity)
    between_EA = create_pose_matrix(0, 0, 0, np.array([0, 0, 0]))
    graph.add_between_factor("E", "A", between_EA)

    # 4. 초기 추정치에 노이즈 추가 (모든 노드에)
    ...
    for node_id in ["B", "C", "D", "E"]:
        ...
        initial_pose[:3, :3] = oplus(initial_pose[:3, :3], noise_rot)
